...
val fold : 'edn value -> ('k, 'edn option) Fun.args -> k:'k -> ctx -> ctx
...
(** [repr protocol] gives a module definition with an OCaml constructor to help
    ...
    {[
      module Protocol
        : Mimice.Protocol.S with type flow = Miou.file_descr

      let edn, protocol = Mimic.register ~name:"protocol" (module Protocol)
      module R = (val (Mimice.repr protocol))

      let () = match Mimic.resolve ~ctx with
        | Ok (R.T file_descr) -> ...
        | ...
    ]} *)

val resolve : ctx -> (flow, [> error ]) result
...
val unfold : ctx -> (edn list, [> `Cycle ]) result
...
val connect : edn list -> (flow, [> error ]) result
